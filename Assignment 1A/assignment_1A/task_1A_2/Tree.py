...
class Node:
    ...
    def __init__(self, name, cat):
        ...
        self.ancestor = None
        self.descendants = []
        self.inherent_prob = None

    def calculate_inherent_prob(self):
        if self.inherent_prob is not None:
            return
        if not self.ancestor:
            if type(self.cat) == list:
                self.cat = np.array(self.cat)
            debug_print(f"Cat for root is {type(self.cat)}")
            self.inherent_prob = self.cat.transpose()
        else:
            self.ancestor.calculate_inherent_prob()
            debug_print(f"The ancestor i_prob of {self.name} is {self.ancestor.inherent_prob}")
            try:
                self.inherent_prob = (self.cat.T * self.ancestor.inherent_prob).T.sum(axis=0)
                debug_print("cat was array")
            except:
                self.inherent_prob = (np.array(self.cat).T * self.ancestor.inherent_prob).T.sum(axis=0)
            debug_print(f"The i_prob of {self.name} is {self.inherent_prob}")
            debug_print(f"With a prob sum of {sum(self.inherent_prob)}")
            debug_print()
...
class Tree:
    ...
    def __init__(self):
        ...
        self.newick = ""
        self.inherent_prob_calculated = False

    def calc_inherent_prob(self):
        if self.inherent_prob_calculated:
            return
        if self.num_leaves > 0:
            print("Calculating inherent probabilities... ")
            ...
            while len(visit_list) != 0:
                ...
                if len(cur_node.descendants) != 0:
                    ...
                    cur_node.calculate_inherent_prob()
            print(f"Calculated in {round(1000*(time.time() - start_time), 2)} ms")
        self.inherent_prob_calculated = True

    def get_inherent_prob_array(self):
        if not self.inherent_prob_calculated:
            self.calc_inherent_prob()
        inherent_prob_array = []
        for i in range(self.num_nodes):
            inherent_prob_array.append(np.zeros(self.k))
        ...
        while len(visit_list) != 0:
            ...
            visit_list = visit_list[1:]
            visit_list = visit_list + cur_node.descendants
            inherent_prob_array[int(cur_node.name)] = cur_node.inherent_prob

        return inherent_prob_array
    ...
    def load_tree_from_direct_arrays(self, topology_array, theta_array=[]):
        ...
        while num_nodes < len(topology_array):
            cur_node = visit_list[0]
            visit_list = visit_list[1:]
            children_indices = [i for i in range(len(topology_array)) if topology_array[i] == int(cur_node.name)]
